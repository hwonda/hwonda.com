          ...
          {projectsData.map((yearData) => (
            <div key={yearData.year} className="flex flex-col space-y-6">
              <h3 className="font-paperlogy text-2xl font-bold">
                {yearData.year}년 ({yearData.projects.length})
              </h3>
              <div className="grid grid-cols-2 gap-1.5">
                {yearData.projects.map((project) => (
                  <a
                    ...
                    key={project.id}
                    className="group bg-gray-9 border-gray8 relative aspect-video rounded-lg border shadow-sm transition-all hover:cursor-pointer hover:shadow-md"
                  >
                    <img
